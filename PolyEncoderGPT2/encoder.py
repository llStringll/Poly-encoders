...
class BertPolyModel(BertPreTrainedModel):
  def __init__(self, config, *inputs, **kwargs):
    ...
    try:
      ...
      self.context_fc = nn.Linear(config.n_embd, self.vec_dim)
      self.response_fc = nn.Linear(config.n_embd, self.vec_dim)
    except:
  ...
  def forward(self, context_input_ids, context_segment_ids, context_input_masks,
    ...
    if isinstance(self.bert, DistilBertModel):
      ...
      state_vecs = self.bert(input_ids=context_input_ids, past=None, attention_mask=context_input_masks, token_type_ids=context_segment_ids)[0]  # [bs, length, dim]
    ...
    if isinstance(self.bert, DistilBertModel):
      ...
      state_vecs = self.bert(input_ids=responses_input_ids, past=None, attention_mask=responses_input_masks, token_type_ids=responses_segment_ids)[0]  # [bs, length, dim]
