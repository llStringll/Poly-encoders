...
class SelectionJoinTransform(object):
  ...
  def __call__(self, texts):
    ...
    for text in texts[::-1][:self.max_history]:
      text += self.tokenizer.eos_token # pad token == eos token == sep token for gpt2
      tokenized_dict = self.tokenizer.encode_plus(text,
                                                  text_pair=None,
                                                  add_special_tokens=True,
                                                  max_length=self.max_len,
      ...
      input_ids_list[0:0]=input_ids
      segment_ids_list[0:0]=segment_ids
      input_masks_list[0:0]=input_masks
